...
function saveGETStringToDB(schedule) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            var totalMonthsX2 = dataToSaveToDB.length - 1;  //This adjusts for the addition of the distro profile
                                                            //which is appended to the end of the string
            ...
            con.connect(function (err) {
                ...
                for (i = 0; i < (totalMonthsX2 / 2); i++) {
                    ...
                    var distributionProfile = dataToSaveToDB[totalMonthsX2];

                    console.log("Here is the month, year, qty and distribution profile: "
                        + month + " " + year + " " + quantity + " " + distributionProfile);


                    var sql = "insert into contract_data.contract_deliveries (contract_name, "
                        + "contract_year, contract_month, monthly_deliveries, distribution_profile) "
                        + "values (?)";
                    var values = [dateTime, year, month, quantity, distributionProfile];
